...
#
# АРХИТЕКТУРА:
#   - Основное приложение: http://techbit.su/ (порт 3000 в Docker)
#   - Приложение Resume: http://techbit.su/resume/ (порт 3001 на хосте)
#
# ВАЖНО: Приложение resume должно быть запущено отдельно на порту 3001
#         и слушать на 0.0.0.0:3001 (не localhost:3001)
#
...
echo ""
echo "Архитектура:"
echo "  - Основное приложение: http://techbit.su/ (Docker, порт 3000)"
echo "  - Приложение Resume:   http://techbit.su/resume/ (хост, порт 3001)"
echo ""
echo "⚠️  ВАЖНО: Приложение resume должно быть запущено отдельно на порту 3001"
echo "           и слушать на 0.0.0.0:3001 (не localhost:3001)"
echo "============================================================================="
echo ""
...
check_system_requirements() {
    ...
    # Проверка портов
    print_status "Проверка портов 80, 443 и 3001..."
    ...
    if ss -tlnp 2>/dev/null | grep -q ":3001 " || netstat -tlnp 2>/dev/null | grep -q ":3001 "; then
        print_success "Порт 3001 используется (приложение resume)"
    else
        print_warning "Порт 3001 не используется - убедитесь, что приложение resume запущено"
        print_warning "Приложение resume должно быть запущено на порту 3001 для работы /resume/"
    fi
...
setup_ssl_certificate() {
    ...
    # Проверка готовности основного приложения
    print_status "Проверка готовности основного приложения (app:3000)..."
    for i in {1..10}; do
        if docker compose exec -T nginx wget -q -O- --timeout=3 http://app:3000 >/dev/null 2>&1; then
            print_success "Основное приложение готово к работе"
            break
        fi
        if [[ $i -eq 10 ]]; then
            print_warning "Основное приложение еще не готово, продолжаем проверку..."
        else
            sleep 3
        fi
    done
    
    # Проверка доступности приложения resume на порту 3001
    print_status "Проверка доступности приложения resume (порт 3001)..."
    if docker compose exec -T nginx wget -q -O- --timeout=3 http://host.docker.internal:3001 >/dev/null 2>&1; then
        print_success "Приложение resume доступно на порту 3001"
    else
        print_warning "Приложение resume недоступно на порту 3001"
        print_warning "Убедитесь, что приложение resume запущено и слушает на 0.0.0.0:3001 (не localhost:3001)"
        print_warning "Путь /resume/ будет недоступен до запуска приложения resume"
    fi
    ...
    if [[ "$HTTP_CODE" == "200" ]] || [[ "$HTTP_CODE" == "301" ]] || [[ "$HTTP_CODE" == "302" ]]; then
        ...
        print_status "Проверка доступности основного приложения из контейнера Nginx:"
        docker compose exec -T nginx wget -q -O- --timeout=5 http://app:3000 2>&1 | head -5 || print_warning "Не удалось подключиться к app:3000"
        
        print_status "Проверка доступности приложения resume из контейнера Nginx:"
        docker compose exec -T nginx wget -q -O- --timeout=5 http://host.docker.internal:3001 2>&1 | head -5 || print_warning "Не удалось подключиться к host.docker.internal:3001"
        ...
        print_status "Проверьте, что приложение resume запущено на порту 3001: ss -tlnp | grep 3001"
        return 1
    fi
}
...
main() {
    ...
    echo ""
    print_status "Приложение Resume:"
    print_status "  HTTP:  http://techbit.su/resume/"
    if [[ -f nginx/sites-enabled/default.conf ]] && grep -q "listen 443" nginx/sites-enabled/default.conf; then
        print_status "  HTTPS: https://techbit.su/resume/"
    fi
    print_warning "Убедитесь, что приложение resume запущено на порту 3001 для работы /resume/"
    echo ""
    echo "ПОЛЕЗНЫЕ КОМАНДЫ:"
    echo "  docker compose ps              # Статус контейнеров"
    echo "  docker compose logs -f app     # Логи основного приложения"
    ...
    echo ""
    echo "ПРОВЕРКА ПРИЛОЖЕНИЯ RESUME:"
    echo "  ss -tlnp | grep 3001           # Проверка, что порт 3001 используется"
    echo "  curl http://localhost:3001     # Проверка доступности приложения resume"
    echo "  curl http://techbit.su/resume/ # Проверка через Nginx"
    echo ""
